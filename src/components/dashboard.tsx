...
export function Dashboard() {
  const [period, setPeriod] = useState(30); const [overview, setOverview] = useState<Overview | null>(null); const [history, setHistory] = useState<Point[]>([]); const [stations, setStations] = useState<Station[]>([]); const [nearby, setNearby] = useState<NearbyStation[]>([]); const [changes, setChanges] = useState<Change[]>([]); const [radius, setRadius] = useState(25); const [locationLoading, setLocationLoading] = useState(false); const [locationError, setLocationError] = useState<string | null>(null); const [coords, setCoords] = useState<{ lat: number; lon: number } | null>(null); const [loading, setLoading] = useState(true);

  useEffect(() => { setLoading(true); Promise.all([fetch("/api/fuels/diesel").then((r) => r.json()), fetch(`/api/fuels/diesel/history?days=${period}`).then((r) => r.json()), fetch("/api/prices/cheapest?limit=5").then((r) => r.json()), fetch("/api/prices/changes").then((r) => r.json())]).then(([a, b, c, d]) => { setOverview(a); setHistory(b); setStations(c); setChanges(d); }).catch((error) => console.error("Dashboard loading error:", error)).finally(() => setLoading(false)); }, [period]);
  ...
  useEffect(() => { if (!coords) return; const controller = new AbortController(); fetch(`/api/prices/nearby?lat=${encodeURIComponent(coords.lat)}&lon=${encodeURIComponent(coords.lon)}&radius=${radius}&limit=10`, { signal: controller.signal }).then((r) => { if (!r.ok) throw new Error("Неуспешно зареждане на близките станции"); return r.json(); }).then(setNearby).catch((error) => { if (error.name !== "AbortError") setLocationError("Неуспяхме да заредим близките станции."); }); return () => controller.abort(); }, [coords, radius]);

  const movement = useMemo(() => { if (history.length < 2) return null; const first = history[0].average, last = history.at(-1)!.average; if (!Number.isFinite(first) || first === 0 || !Number.isFinite(last)) return null; return { value: last - first, percent: ((last - first) / first) * 100 }; }, [history]);
  const hasData = overview?.average != null;

  const useLocation = () => { setLocationError(null); if (!("geolocation" in navigator)) { setLocationError("Браузърът не поддържа геолокация."); return; } setLocationLoading(true); navigator.geolocation.getCurrentPosition((position) => { setCoords({ lat: position.coords.latitude, lon: position.coords.longitude }); setLocationLoading(false); }, () => { setLocationLoading(false); setLocationError("Няма разрешение за местоположение. Разреши Location в браузъра и опитай отново."); }, { enableHighAccuracy: true, timeout: 10000, maximumAge: 300000 }); };

  const shownStations = coords ? (nearby.length ? nearby : stations) : [];
  ...
  return <main className="shell">
    ...
    <section className="grid"><article className="panel" id="cheapest"><div className="panel-title"><div><h3>{coords ? "НАЙ-ЕВТИН ДО ТЕБ" : "НАЙ-ЕВТИН ДО ТЕБ"}</h3>{coords ? <small style={{ opacity: 0.7 }}>сортирани по цена · {radius} km</small> : <small style={{ opacity: 0.7 }}>Нуждаем се от твоето местоположение</small>}</div><button type="button" onClick={useLocation} disabled={locationLoading}>{locationLoading ? "Зареждаме…" : "📍 Моето местоположение"}</button></div>
      ...
      {!coords && !locationError && locationLoading ? <div className="empty">◌ Получаваме местоположението ти…</div> : null}
      <div className="station-list">{shownStations.length ? shownStations.map((station, index) => <a key={station.id} className="station" href={station.sourceUrl} target="_blank" rel="noreferrer"><b>{String(index + 1).padStart(2,"0")}</b><div><strong>{station.brand && station.brand !== "gasstation" ? station.brand : station.name}</strong><span>{station.city} · {station.address}{"distanceKm" in station ? ` · ${station.distanceKm} km` : ""}</span></div><div className="station-price"><strong>{fmt.format(station.price)}</strong><span>{age(station.observedAt)}</span></div></a>) : null}</div>
      ...
      {coords && nearby.length === 0 ? <div className="empty">◌ Няма станции с валидна цена в радиус {radius} km.</div> : null}
