...
export function Dashboard() {
  ...
  const marketSignal = useMemo(() => {
    ...
    if (!news.length) return "Няма достатъчно новини";
    if (bad > good) return "По-скоро натиск за поскъпване";
    if (good > bad) return "По-скоро натиск за поевтиняване";
    return "Смесени пазарни сигнали";
  }, [news]);
  ...
  const useLocation = () => {
    ...
    navigator.geolocation.getCurrentPosition(
      ...
      () => {
        setLocationLoading(false);
        setLocationError("Няма разрешение за местоположение. Разреши Location и опитай отново.");
      },
    ...
    <main className="shell">
      <header>
        ...
        <nav>
          ...
          <a href="#cheapest">Най-евтин</a>
          <a href="#changes">Промени</a>
          <a href="#forecast">Прогноза</a>
          <a href="/admin">Админ</a>
        </nav>
        <button className="live"><b /> Данни на живо</button>
      </header>

      <section className="fuel-selector" aria-label="Избор на гориво">
        <div>
          <p className="eyebrow">ИЗБЕРИ ГОРИВО</p>
          <h2>{fuelLabel(fuel)}</h2>
          <small>Избери горивото, за което искаш да виждаш цените, историята и най-евтините станции.</small>
        </div>
        <div className="fuel-options">
          {FUEL_OPTIONS.map((option) => {
            const selected = fuel === option.key;
            return (
              <button key={option.key} type="button" onClick={() => setFuel(option.key)} aria-pressed={selected} className={selected ? "selected" : ""}>
                {option.label}
                {option.key === "diesel" ? <span>по подразбиране</span> : null}
              </button>
            );
          })}
        </div>
      ...
      <section className="hero" id="overview">
        <div>
          <p className="eyebrow">БЪЛГАРИЯ · {fuelLabel(fuel).toUpperCase()}</p>
          <h1>Цената на <em>{fuelLabel(fuel).toLowerCase()},</em><br />без догадки.</h1>
          <p className="lede">Показваме проследими цени с посочен източник и време на наблюдение.</p>
        </div>
        <div className="hero-chip">
          <span>Надеждност</span>
          <strong>{overview?.confidence ?? "—"}{overview?.confidence != null && "%"}</strong>
          <small>{overview?.stationCount ?? 0} валидни обекта</small>
        </div>
      </section>
      ...
      <section className="primary-card">
        <div className="price-head">
          <div>
            <p>СРЕДНА ЦЕНА · {fuelLabel(fuel).toUpperCase()}</p>
            <h2>{hasData ? fmt.format(overview!.average!) : "Няма данни"}<small>{hasData && " / литър"}</small></h2>
            <div className={movement && movement.value >= 0 ? "movement up" : "movement down"}>
              {movement ? `${movement.value >= 0 ? "▲" : "▼"} ${fmt.format(Math.abs(movement.value))} · ${Math.abs(movement.percent).toFixed(1)}% за избрания период` : "Няма достатъчно история за тренд"}
            </div>
          </div>
          <div className="fresh">
            <span className={overview?.latest ? "dot fresh" : "dot"} />Последно обновяване: <b>{age(overview?.latest ?? null)}</b>
            <small>{overview?.sourceCount ?? 0} потвърдени източника</small>
          </div>
        </div>
        <div className="periods">
          {[[1, "24ч"], [7, "7 дни"], [30, "30 дни"], [90, "3 мес"], [365, "1 год"]].map(([value, label]) => (
            <button key={String(value)} onClick={() => setPeriod(Number(value))} className={period === value ? "selected" : ""}>{label}</button>
          ))}
        </div>
          ...
          {history.length ? (
            <ResponsiveContainer width="100%" height={290}>
              <AreaChart data={history}>
                <defs><linearGradient id="fuel" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor="#bfef4b" stopOpacity=".36" /><stop offset="100%" stopColor="#bfef4b" stopOpacity="0" /></linearGradient></defs>
                ...
                <Tooltip contentStyle={{ background: "#15201f", border: "1px solid #30413e", borderRadius: 10 }} formatter={(value) => fmt.format(Number(value))} />
                <Area type="monotone" dataKey="average" stroke="#c6f44d" strokeWidth={3} fill="url(#fuel)" />
              </AreaChart>
            </ResponsiveContainer>
          ) : <Empty label={loading ? `Зареждаме ${fuelLabel(fuel)}…` : `Все още няма валидирани наблюдения за ${fuelLabel(fuel)}.`} />}
        </div>
      </section>

      <section className="stats">
        {[["Средна", overview?.average], ["Най-ниска", overview?.lowest], ["Най-висока", overview?.highest], ["Медианна", overview?.median]].map(([label, value]) => (
          <article key={String(label)}><span>{label}</span><strong>{typeof value === "number" ? fmt.format(value) : "—"}</strong><small>без аномални стойности</small></article>
        ))}
      </section>
      ...
      <section className="grid">
        <article className="panel" id="cheapest">
          <div className="panel-title">
            <div><h3>НАЙ-ЕВТИН ДО ТЕБ · {fuelLabel(fuel).toUpperCase()}</h3>{coords ? <small>сортирани по цена · {radius} km</small> : <small>Нуждаем се от твоето местоположение</small>}</div>
            <button type="button" onClick={useLocation} disabled={locationLoading}>{locationLoading ? "Зареждаме…" : "📍 Моето местоположение"}</button>
          </div>
          <div className="periods">
            {[5, 10, 25, 50].map((value) => <button key={value} onClick={() => setRadius(value)} className={radius === value ? "selected" : ""}>{value} km</button>)}
          </div>
          ...
          <div className="station-list">
            {nearby.map((station, index) => (
              <a key={station.id} className="station" href={station.sourceUrl} target="_blank" rel="noreferrer">
                <b>{String(index + 1).padStart(2, "0")}</b>
                <div><strong>{station.brand && station.brand !== "gasstation" ? station.brand : station.name}</strong><span>{station.city} · {station.address} · {station.distanceKm.toFixed(2)} km</span></div>
                <div className="station-price"><strong>{fmt.format(station.price)}</strong><span>{age(station.observedAt)}</span></div>
              </a>
            ))}
          </div>
          ...
          {coords && !nearbyLoading && !nearby.length && !locationError ? <div className="empty">◌ Няма станции с валидна цена в радиус {radius} km.</div> : null}
          <LocationMap latitude={coords?.lat ?? null} longitude={coords?.lon ?? null} radiusKm={radius} stations={nearby} />
        </article>

        <article className="panel" id="changes">
          <div className="panel-title"><div><h3>ПОСЛЕДНИ ПРОМЕНИ</h3><small>{marketSignal}</small></div><a href="#forecast">Прогноза →</a></div>
          {changes.length ? (
            <div className="change-list">
              {changes.slice(0, 3).map((change) => (
                <a key={change.id} className="change" href={change.sourceUrl} target="_blank" rel="noreferrer">
                  <div className={change.change >= 0 ? "arrow rise" : "arrow fall"}>{change.change >= 0 ? "↑" : "↓"}</div>
                  <div><strong>{change.station} <span>· {change.city}</span></strong><small>{fmt.format(change.oldPrice)} → {fmt.format(change.newPrice)} · {age(change.detectedAt)}</small></div>
                  <b className={change.change >= 0 ? "rise" : "fall"}>{change.change >= 0 ? "+" : ""}{fmt.format(change.change)}</b>
                </a>
              ))}
            </div>
          ) : <Empty label="Няма регистрирани промени за избраното гориво." />}
          {news.length ? (
          ...
          ) : <Empty label="Все още няма събрани новини за пазара на горива." />}
        </article>
      </section>
