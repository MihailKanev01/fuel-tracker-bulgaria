...
export function DieselForecast() {
  ...
  useEffect(() => {
    ...
    fetch("/api/fuels/diesel/forecast", { signal: controller.signal })
      .then((response) => {
  ...
  if (loading) return <div className="forecast-panel panel"><div className="forecast-empty">◌ Изчисляваме прогнозата от текущите пазарни данни…</div></div>;
  if (!data?.points.length || data.current == null) return <div className="forecast-panel panel"><div className="forecast-empty">◌ Няма достатъчно проверени данни за надеждна прогноза.</div></div>;

  return <section className="forecast-panel panel" id="forecast">
    <div className="forecast-head">
      <div>
        <p className="eyebrow">ОЧАКВАНА ЦЕНА · DIESEL</p>
    ...
    <div className="forecast-metrics">
      ...
      <div><span>Ден 7</span><strong>{fmt.format(data.expectedEnd ?? data.current)}</strong></div>
    ...
    <div className="forecast-factors">
      {data.factors.map((factor) => <span key={factor.label} className={`factor ${factor.direction.toLowerCase()}`}>
        {factor.direction === "UP" ? "↑" : factor.direction === "DOWN" ? "↓" : "→"} {factor.label}
      </span>)}
    </div>

    <p className="forecast-note">{data.explanation} Прогнозата се преизчислява при следващото обновяване на данните и постепенно се разширява като диапазон с увеличаване на хоризонта.</p>
  </section>;
}
