...
export type ForecastPoint = { date: string; low: number; expected: number; high: number; confidence: "HIGH" | "MEDIUM" | "LOW" };
...
export type DieselForecast = FuelForecast;
...
export async function fuelForecast(fuelType: FuelType): Promise<FuelForecast> {
  const [historyRows, latestRows, newsRows, brent] = await Promise.all([
    prisma.price.findMany({
      where: { fuelType, anomaly: false, observedAt: { gte: new Date(Date.now() - 30 * DAY) } },
      select: { priceEur: true, observedAt: true }, orderBy: { observedAt: "asc" },
    }),
    prisma.price.findMany({
      where: { fuelType, anomaly: false },
      select: { priceEur: true, observedAt: true }, orderBy: { observedAt: "desc" }, take: 500,
    }),
    prisma.newsItem.findMany({
      where: { publishedAt: { gte: new Date(Date.now() - 2 * DAY) } },
      select: { impact: true }, orderBy: { publishedAt: "desc" }, take: 30,
    }),
    fetch("https://query1.finance.yahoo.com/v8/finance/chart/BZ=F?range=14d&interval=1d", {
      headers: { Accept: "application/json", "User-Agent": "FuelTrackerBG/1.0" }, cache: "no-store", signal: AbortSignal.timeout(8_000),
    }).then(async (response) => {
  ...
  if (current == null) return {
    generatedAt: new Date().toISOString(), horizonDays: 7, current: null, expectedEnd: null, expectedLow: null, expectedHigh: null,
    confidence: "LOW", direction: "FLAT", explanation: "Няма достатъчно проверени данни за надеждна прогноза.", factors: [], points: [],
  };
  ...
  const confidence: FuelForecast["confidence"] = daily.length >= 14 && volatility < 0.008 ? "HIGH" : daily.length >= 7 ? "MEDIUM" : "LOW";
  const fuelLabels: Record<FuelType, string> = { DIESEL: "Diesel", GASOLINE_95: "A95", GASOLINE_100: "A100", LPG: "LPG", CNG: "CNG" };
  const label = fuelLabels[fuelType];
  const factors: FuelForecast["factors"] = [
    { label: `Местна тенденция · ${label}`, direction: recentTrend > 0.00025 ? "UP" : recentTrend < -0.00025 ? "DOWN" : "NEUTRAL", weight: 50 },
  ...
  for (let day = 0; day <= 7; day += 1) {
    ...
    points.push({ date, low: Math.max(0, expected - spread), expected, high: expected + spread, confidence: day <= 2 ? "HIGH" : day <= 5 ? "MEDIUM" : "LOW" });
  }
  const end = points.at(-1)!;
  const explanation = direction === "UP"
    ? `Базовият сценарий за ${label} е за умерено поскъпване. Местната динамика е подкрепена от поскъпващ петрол или негативни пазарни новини.`
    : direction === "DOWN"
      ? `Базовият сценарий за ${label} е за умерено поевтиняване. Местната динамика е подкрепена от по-слаб петролен пазар или позитивни пазарни новини.`
      : `Базовият сценарий за ${label} е за относително стабилна цена. Наличните сигнали не дават достатъчно силна посока за голямо движение.`;

  return { generatedAt: new Date().toISOString(), horizonDays: 7, current, expectedEnd: end.expected,
    expectedLow: Math.min(...points.map((point) => point.low)), expectedHigh: Math.max(...points.map((point) => point.high)),
    confidence, direction, explanation, factors, points };
}

export async function dieselForecast() { return fuelForecast("DIESEL"); }
