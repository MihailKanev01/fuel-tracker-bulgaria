...
async function fetchBoundsBox(bounds: Bounds, zoom: number) { const form = new URLSearchParams({ lat_max: String(bounds.maxLat), lon_max: String(bounds.maxLon), lat_min: String(bounds.minLat), lon_min: String(bounds.maxLon), zoom: String(zoom), country: "bg", fuel: "all", brand: "all" }); return fueloJson<FueloBoundsResponse>(FUELO_MAP_URL, { method: "POST", headers: { "Content-Type": "application/x-www-form-urlencoded; charset=UTF-8" }, body: form }); }
...
function parseInfo(marker: FueloMarker, info: FueloInfoResponse, fuelType: FuelType) {
  ...
  const fuelPatterns = labels[fuelType].map((label) => new RegExp(`title=[\"']${label}:?(?:\\s*([0-9]+(?:[.,][0-9]+)?)\\s*€/л)?[\"']`, "i"));
  const matchingTitle = fuelPatterns.find((pattern) => pattern.test(html));
  const price = matchingTitle
    ? labels[fuelType].map((label) => html.match(new RegExp(`title=[\"']${label}:\\s*([0-9]+(?:[.,][0-9]+)?)\\s*€/л`, "i"))?.[1]).map(parseNumber).find((value): value is number => value != null && value > 0) ?? null
    : null;
  const latitude = parseNumber(marker.lat); const longitude = parseNumber(marker.lon);
  if (!name || !location || !matchingTitle || latitude == null || longitude == null) return null;
  const parts = location.split(",").map((part) => part.trim()).filter(Boolean);
  return { id: `fuelo-${marker.id}`, name, brand: normalizeBrand(marker.logo), city: parts[parts.length - 1] ?? "България", address: parts.slice(1).join(", ") || parts[parts.length - 1] || "България", price, observedAt: new Date(), confidence: price != null ? 75 : 50, sourceUrl: `https://bg.fuelo.net/gasstation/id/${encodeURIComponent(marker.id)}?lang=bg`, latitude, longitude, fuelType };
}
...
async function liveFueloNearby(latitude: number, longitude: number, radiusKm: number, limit: number, fuelType: FuelType) {
  try {
    ...
    return results.sort((a, b) => (a.price ?? Number.POSITIVE_INFINITY) - (b.price ?? Number.POSITIVE_INFINITY) || a.distanceKm - b.distanceKm).slice(0, limit);
  } catch { return []; }
}
async function databaseNearby(latitude: number, longitude: number, radiusKm: number, limit: number, fuelType: FuelType): Promise<NearbyFuelStation[]> {
  ...
  return stations.flatMap((station) => {
    ...
    return [{ id: station.id, name: station.name, brand: station.brand, city: station.city, address: station.address, price: price?.priceEur.toNumber() ?? null, observedAt: price?.observedAt ?? new Date(0), confidence: price?.confidence ?? 0, sourceUrl: price?.originalUrl ?? `https://bg.fuelo.net/gasstation/id/${encodeURIComponent(station.id.replace(/^fuelo-/, ""))}?lang=bg`, latitude: stationLat, longitude: stationLon, distanceKm: Number(distanceKm.toFixed(2)) }];
  }).sort((a, b) => (a.price ?? Number.POSITIVE_INFINITY) - (b.price ?? Number.POSITIVE_INFINITY) || a.distanceKm - b.distanceKm).slice(0, limit);
}
export async function nearbyFuel(latitude: number, longitude: number, radiusKm = 25, limit = 10, fuelType: FuelType = "DIESEL") {
  ...
  return [...merged.values()].sort((a, b) => (a.price ?? Number.POSITIVE_INFINITY) - (b.price ?? Number.POSITIVE_INFINITY) || a.distanceKm - b.distanceKm).slice(0, Math.max(limit, 1000));
}
